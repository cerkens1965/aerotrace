...
- 2026-09-22 11h30 : FLEET d'après Claude Design (export `design_handoff_airki/AirKi Dashboard-2/Fleet health dashboard/Fleet.dc.html`), priorité VISIBILITÉ — en-tête (SIM DATA SYNCED … · Refresh), 4 MetricCard avec statut (coût / heure de vol), panneau encre NEEDS ATTENTION (boîtier muet ≥ 3 j, mise à jour échouée, version en retard AKcore/AKview, pas d'avion lié, conso > 2× la médiane MB/h) avec bouton Open/Link, tableau 8 colonnes gros chiffres (UNIT · STATE · AIRCRAFT · FIRMWARE AKC/AKV empilés + « → n » · CHANNEL FLEET/DEV · WIFI + nb réseaux · DATA MB + € · MB/H · LAST SEEN + âge), filtre ALL / NEEDS ATTENTION, pied « PUBLISHED NOW … » ; tiroir : identité (fiche avion, lecture seule) ou saisie directe, canal Unchanged/Fleet/Dev (bascules ; un simple changement de canal s'enregistre), WiFi club avec Show/Hide, réseaux connus en cartes (Edit / Forget / Keep, n OF 6), « Save & push to box ». Supprimés : bandeaux DATA · EMNIFY et VERSIONS (détails en infobulle et en pied). Prévisualisation.
- 2026-09-22 11h35 : POUSSÉ EN PROD — Fleet Claude Design.
- 2026-09-22 12h : ADMIN d'après Claude Design (export `design_handoff_airki/Admin dashboard live build/Admin.dc.html`) — en-tête + onglets, 3 MetricCard par onglet (Pilots : on file / account not linked / duplicate PIN ; Aircraft : on file / no hex / no photo ; Access : members / pending / pilots without access), barre recherche « n OF total » + Purge archived + New, tableaux actifs (statut par ligne : LINKED/NOT LINKED, DUPLICATE, NO HEX/NO PHOTO/COMPLETE) avec Open / Archive, section ARCHIVÉS séparée (bannière archive ≠ purge, REFERRED TO BY n FLIGHTS · KEPT ON PURGE / CAN BE PURGED — vols du club chargés une fois), bannière de notification après archive/restore/rôle/retrait. Tiroir pilote : Student/Licensed/FI en bascules, section INVITATION CODE (1 GENERATE → 2 COPY → 3 PILOT SIGNS IN → 4 LINKED, code en grand sur encre, Copy/Replace), pied « Archive pilot ». Tiroir avion : types rapides (types du club), Club/Private owner en bascules, pied « Archive aircraft ». Access : formulaire « Invite someone » toujours visible à côté des invitations en attente. PIN toujours masqué (••••). Prévisualisation.
- 2026-09-22 : FLEET « NEEDS ATTENTION » tenable à 500 boîtiers (demande Christophe) — UPDATE AVAILABLE n'est PLUS une alerte (état normal d'un déploiement, `routine: true`) : une simple ligne « n UNITS WAITING FOR AN UPDATE · NOT AN ISSUE » + filtre tableau UPDATE PENDING. Panneau AGRÉGÉ par type (UPDATE FAILED · NOT SEEN ≥3 D · NO AIRCRAFT LINKED · HIGH DATA USE) : compteur + explication + 8 puces boîtier max (clic = fiche) + « +n MORE » + « Show these n » (filtre le tableau par type). `showOnly` = null | 'attention' | clé de problème. Build OK. POUSSÉ EN PROD 22/09 (« pousse en prod »), non testé navigateur. Déploiement : `npx -y firebase-tools deploy --only hosting` depuis aerotrace/ (pas de CLI globale).
- 2026-09-22 17h : POUSSÉ EN PROD — Admin Claude Design.
- 2026-09-22 20h : RECHERCHE sur toutes les listes — `components/ui/SearchBox.jsx` (loupe, Échap efface, « n OF total ») + `utils/search.js` (fold/matches, sortis d'AdminPage) : Logbook (vols : avion, pilote, instructeur, terrain, type, date ; cartes Pilots/Instructors/Aircraft), Fleet (boîtier, avion, versions, canal, WiFi, état, SIM — combinée aux filtres NEEDS ATTENTION / UPDATE PENDING), Admin (inchangée, migrée sur le composant partagé). All flights : groupes DAY/AIRCRAFT/PILOT désormais REPLIABLES (ligne + chevron + totaux, fermés par défaut ; tout ouvert à plat ou pendant une recherche) — demande Christophe « comme dans le logbook ». Prévisualisation.
- 2026-09-22 20h15 : POUSSÉ EN PROD — recherche sur toutes les listes + groupes All flights repliables (+ panneau Fleet agrégé par type de problème, modifié hors de mes commits).
- 2026-09-23 soir : MOBILE (tablette, iPhone, iPhone Duo) — POUSSÉ EN PROD. `src/hooks/useBreakpoint.js` = seuil unique (phone < 640, tablet < 1024, desktop ≥ 1024, matchMedia) : on raisonne en LARGEUR, jamais en appareil (un pliable traverse le seuil comme une rotation). LIVE d'après Claude Design (brief `01 - Documentation/design_handoff_airki/Brief 06 - Live on iPad, iPhone and iPhone Duo.md`, export `design_handoff_airki/Live on ipadiphone/`) : sous 1024 px les panneaux flottants LAYERS et la colonne In flight deviennent une FEUILLE appelée (`components/map/MapSheet.jsx`, onglets Fleet/Layers, crans 380/plein sur téléphone, ancrée 360 px en bas à droite sur tablette) + deux boutons 44 px à hauteur de pouce ; bandeau flotte d'UNE ligne en lavis (`components/map/FleetStripWash.jsx`) ; NavigationControl (+/−) retiré sous 1024 px (on pince). Toute surface posée sur la carte = lavis papier 18 % + flou 8 px + filet 1 px (règle 8b). Monogramme A duo 20 px en bas à gauche, même place sur toutes les pages.
- ⚠️ DEUX PIÈGES RENCONTRÉS LE 23/09, à chercher en premier sur toute page mobile :
  1. **Un sous-composant déclaré AU NIVEAU MODULE ne voit pas la palette du composant.** Switch/SliderTrack/BandSlider (au niveau module obligatoirement, sinon le glisser casse) et Figure/FlightCard de `LiveInFlightPanel` dessinaient en dur pour le panneau ENCRE → texte et poignées blancs sur le lavis clair. Correctif : palette passée EN PROP (`pal`, `P`), défaut encre (`INK_PAL`). Test : tout `T.white` / `T.mutedDark` hors d'une table de palette est suspect.
  2. **Un `/* … */` en position d'ENFANT JSX est du TEXTE, pas un commentaire** — il s'est affiché en gros sur la carte Live. En attribut ou dans une accolade, il est correct.
- 2026-09-23 soir : DataTable rend une CARTE PAR LIGNE sous 640 px (le repli overflow-x faisait perdre la colonne d'identité à chaque geste). Trois options par colonne, sans effet sur la table : `phone: 'hide'`, `phone: 'minor'` (ligne dense en pied de carte), `phoneRender(row)` (rendu court, une ligne au lieu de deux). Posées sur le Logbook (« too much info pour l'iPhone ») : date · heure sur une ligne, avion · type, terrain, pilote, durée ; type de vol, instructeur et statut en pied. Fleet et Admin peuvent s'alléger de la même façon.
- 2026-09-23 soir : DÉFILEMENT EN BIAIS (Logbook, Fleet) — un conteneur de défilement SANS largeur bornée s'élargit au moindre débordement et la page glisse en diagonale. Les quatre pages (Logbook, Fleet, Admin, In flight) portent `width/maxWidth: '100%'` + `overflowX: 'hidden'` et des marges proportionnelles `clamp(14px, 3.5vw, 28px)`. In flight l'avait depuis le début, d'où la comparaison de Christophe.
- 2026-09-24 : BANC D'ESSAI SUR SIMULATEUR (nouveau, et il servira au durcissement des règles). `firebase.json` porte la config `emulators` (auth 9099, functions 5001, firestore 8080, UI désactivée). Le simulateur Firestore exige **Java 21** — `openjdk@21` installé par brew, keg-only : lancer avec `JAVA_HOME=/opt/homebrew/opt/openjdk@21 PATH="$JAVA_HOME/bin:$PATH"` (le PATH système garde Java 8, rien n'est modifié ailleurs). Deux terminaux : `npm run emulators` (depuis functions/), puis `npm run test:coownership`. 23 vérifications de bout en bout : trigger de normalisation réel, transaction de revendication, jetons d'authentification réels sur la fonction appelable.
- 2026-09-24 : COPROPRIÉTÉ (schéma + cloud, NON DÉPLOYÉ à ce stade). Fiche aéronef : `ownerPilotIds` (tableau) devient la source, `ownerPilotId` reste tenu sur le premier nom (tout le dashboard le lit encore) ; conversion automatique par le trigger `syncAircraftIdentity` à la première écriture, ou `backfillOwnerIds` (callable admin) d'un coup. Règles d'attribution sorties dans `functions/attribution.js` — PURES et testées par `npm test` (18 vérifications) : ces heures finissent dans des carnets de vol, elles ne doivent pas dépendre d'une expression qu'on ne peut vérifier qu'en déployant. Un seul propriétaire = inchangé ; plusieurs = REVENDICATION, personne crédité ; un code saisi à l'avion prime et vaut validation si le pilote est copropriétaire. Nouveau champ `pilotSource` ('declared' | 'owner' | 'claimed' | 'assigned') : le carnet ne présente jamais une déduction comme un fait. `claimFlight` (callable) = seul chemin d'écriture pour un pilote (les règles réservent /flights aux instructeurs et admins) : premier arrivé premier servi, seconde revendication = conflit tracé, `mine:false` récuse ou REND un vol auto-attribué (propriétaire qui prête son appareil). Le boîtier reçoit `owners` (trigrammes de tous les copropriétaires) ; `owner` (pilote par défaut) n'est renseigné que s'il n'y en a qu'un. Reste : écran de revendication dans le carnet, et la question « qui pilote ? » sur l'AKview.
