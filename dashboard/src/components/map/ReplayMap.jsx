...
export default function ReplayMap({ frames, currentFrame, is3D = false }) {
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      if (ghostCoords.length > 1) { // always show full ghost trace
        ...
        map.addLayer({
          ...
          paint: { 'line-color': '#ffffff', 'line-width': 2, 'line-opacity': 0.25, 'line-dasharray': [2, 1] }
        })
      }
      ...
      `
      el.innerHTML = '<img src="/icons/VL3.svg" style="width:100%;height:100%;" />'
      markerRef.current = new maplibregl.Marker({ element: el, rotationAlignment: 'map' })
      headRef.current = 0

      // Ensure ghost trace visible immediately
      // Fit to trace bounds
