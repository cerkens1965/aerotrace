...
function AirborneCard({ ac, owner, now, onLocate }) {
  ...
  if (lte) {
    ...
    source = `AKcore ONLY${lastAlt || lastGs ? ` · LAST ${[lastAlt, lastGs].filter(Boolean).join(' / ')}` : ''}${f?.lastSeen ? `, ${mmss(silent)} AGO` : ''}`
  } else {
    ...
    source = l ? `SAFESKY NETWORK${age != null ? ` · ${age} S AGO` : ''}` : 'AKcore'
  }
...
export default function EnVolPage() {
  ...
  const silentNote = (ac) => {
    ...
    return keys.some(k => k in boxSeen) ? 'NO SIGNAL RECEIVED' : 'NO AKcore LINKED'
  }
